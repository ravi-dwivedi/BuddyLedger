...
import com.ravi.android.buddy.ledger.StartupSecurityActivity;
import com.ravi.android.buddy.ledger.data.DatabaseCreator;
...
public class SettingsFragment extends PreferenceFragment implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Intent intent = new Intent(Intent.ACTION_SENDTO, Uri.fromParts("mailto", "ravi.dwivedi23@gmail.com", null));
        intent.putExtra(Intent.EXTRA_SUBJECT, "Buddy Ledger App on PlayStore");
        ...
        Intent change = new Intent(getActivity(), StartupSecurityActivity.class);
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String s) {
        if (s.equals("securitySwitch")) {
            if (sharedPreferences.getBoolean("securitySwitch", false)) {
                Intent savePassword = new Intent(getActivity(), StartupSecurityActivity.class);
                savePassword.setAction("SAVE_PASSWORD");
                startActivity(savePassword);
            } else {
                if (!sharedPreferences.getString("password", "abcd").equals("abcd")) {
                    Intent removePassword = new Intent(getActivity(), StartupSecurityActivity.class);
        ...
        try {
            ...
            if (sd.canWrite()) {
                String newDBPath = "//data//" + getActivity().getPackageName()
                        + "//databases//" + DatabaseCreator.DATABASE_NAME;
                String oldDBPath = ApplicationGlobals.storedDBPath; // From SD directory.
    ...
    private void exportDB() {
        ...
        try {
            ...
            if (sd.canWrite()) {
                ...
                String backupDBPath = ApplicationGlobals.storedDBPath;
                File currentDB = getActivity().getDatabasePath(DatabaseCreator.DATABASE_NAME);
